...
class VOCriterion:
    def __init__(self, t_crit='rms', rot_crit='none', flow_crit='none', target_t_crit='none',
        ...
        elif t_crit == 'roni_lior':
            self.calc_t_crit = self.calc_new_loss
        elif t_crit == 'roni_lior_target':
            self.calc_t_crit = self.calc_new_loss_with_target
        elif t_crit == "mean_loss":
            self.calc_t_crit = self.calc_mean_loss
        elif t_crit == "target_reg":
            self.calc_t_crit = self.calc_cumul_poses_t
            self.calc_target_t_product = True
        elif t_crit == "reg_rotation":
            self.calc_t_crit = self.calc_cumul_poses_t
            self.calc_rot_crit = self.calc_rot_quat_product
        elif t_crit == "reg_rotation_target":
            self.calc_t_crit = self.calc_cumul_poses_t
            self.calc_rot_crit = self.calc_rot_quat_product
            self.calc_target_t_product = True
        else:
        ...
        # self.calc_rot_crit = self.calc_rotation_crit

        self.calc_flow_crit = self.calc_none

        #
        # self.calc_target_t_product = True
        self.arccos_min = None
        self.arccos_max = None

    def apply(self, model_output, scale, motions_gt, target_pose, flow_clean=None):
        motions, flow = model_output
        self.arccos_min = torch.tensor(-1, device= motions.device)
        self.arccos_max = torch.tensor(1, device= motions.device)
    ...
    def calc_rotation_crit(self,motions, motions_gt):
        ...
        r_err, target_r = self.rotation_error(cumul_poses, cumul_poses_gt)

        return r_err

    def calc_new_loss(self, motions, motions_gt, target_pose):
        rel_poses = self.rtvec_to_pose(motions)
        rel_poses_gt = self.rtvec_to_pose(motions_gt)
        cumul_poses = self.cumulative_poses(rel_poses)
        cumul_poses_gt = self.cumulative_poses(rel_poses_gt)
        r_err, target_r = self.rotation_error(cumul_poses, cumul_poses_gt)
        t_err, target_t = self.translation_error( cumul_poses, cumul_poses_gt, target_pose)
        loss = r_err + t_err
        return loss, 0

    def calc_new_loss_with_target(self, motions, motions_gt, target_pose):
        rel_poses = self.rtvec_to_pose(motions)
        cumul_poses = self.cumulative_poses(rel_poses)
        inverted = torch.zeros_like(cumul_poses)
        for i, pose in enumerate(cumul_poses):
            inverted[i] = torch.linalg.inv(pose)@ target_pose
        r_err, target_r = self.rotation_error_target(inverted)
        t_err, target_t = self.translation_error_target(inverted)
        loss = r_err + t_err
        return loss, 0

    def rotation_error(self, cumul_poses, cumul_poses_gt):

        r_error = torch.zeros(len(cumul_poses), device=cumul_poses.device)
        r_error_gt = torch.zeros(len(cumul_poses), device=cumul_poses.device)
        for i,(pose,pose_gt) in enumerate(zip(cumul_poses,cumul_poses_gt)):
            r_error[i] = self.rotation_error_new(pose)
            r_error_gt[i] = self.rotation_error_new(pose_gt)

        total_error = r_error
        r_target_error = 0

        return total_error[1:], r_target_error

    def rotation_error_target(self, rel):

        r_error = torch.zeros(len(rel), device=rel.device)
        for i, pose in enumerate(rel):
            r_error[i] = self.rotation_error_new(pose)
    ...
    def rotation_error_new(self, pose_error):
    ...
    def calc_mean_loss(self, motions, motions_gt, target_pose):
        t_error_rms, target_t_error_rms = self.calc_cumul_poses_t(motions, motions_gt, target_pose)
        t_errors_mean, target_t_errors_mean = self.calc_mean_partial_poses_t(motions, motions_gt, target_pose)
        weighted_avg = 0.7*t_error_rms + 0.3*t_errors_mean
        weighted_avg_target = 0.7*t_errors_mean + 0.3*target_t_errors_mean
        return weighted_avg, weighted_avg_target
    ...
    # def translation_error(self, cumul_poses, cumul_poses_gt, target):
    #
    #     t_error = torch.zeros(len(cumul_poses), device=cumul_poses.device)
    #     t_error_gt = torch.zeros(len(cumul_poses), device=cumul_poses.device)
    #     for i,(pose,pose_gt) in enumerate(zip(cumul_poses,cumul_poses_gt)):
    #         t_error[i] = self.translation_error_new(pose)
    #         t_error_gt[i] = self.translation_error_new(pose_gt)
    #
    #     total_error = t_error
    #     t_target_error = 0
    #
    #     return total_error, t_target_error

    def translation_error_target(self, rel):

        t_error = torch.zeros(len(rel), device=rel.device)
        for i, (pose, pose_gt) in enumerate(rel):
            t_error[i] = self.translation_error_new(pose)
    ...
    def translation_error_new(self, pose_error):
