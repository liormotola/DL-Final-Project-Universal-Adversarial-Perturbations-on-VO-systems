...
class Attack:
    def __init__(self, model, criterion, test_criterion, norm, data_shape,mean_grad_beta,
        ...
        self.mean_grad = 0
        self.mean_grad_beta = mean_grad_beta
        self.momentum = 0
        self.beta_one = 0.9
        self.beta_two = 0.999
        self.epsilon = 1e-8
    ...
    def calc_sample_grad_APGD(self, pert, img1_I0, img2_I0, intrinsic_I0, img1_delta, img2_delta,
                         scale, y, clean_flow, target_pose, perspective1, perspective2, mask1, mask2, device=None):
        grad , loss = self.calc_sample_grad_single_APGD(pert, img1_I0, img2_I0, intrinsic_I0, img1_delta, img2_delta,
                         scale, y, clean_flow, target_pose, perspective1, perspective2, mask1, mask2, device)
        return grad, loss

    def calc_sample_grad_single_APGD(self, pert, img1_I0, img2_I0, intrinsic_I0, img1_delta, img2_delta,
                         scale, y, clean_flow, target_pose, perspective1, perspective2, mask1, mask2, device=None):
    ...
    def gradient_ascent_momentum_step(self, pert, data_shape, data_loader, y_list, clean_flow_list,
                             multiplier, a_abs, eps,old_grad = None, device=None):
        ...
        with torch.no_grad():
            ...
            mu = 0.95
            grad += mu*old_grad
            pert += a_abs * torch.sign(grad)
            pert = self.project(pert, eps)

        return pert, grad


    def gradient_ascent_rmsprop_step(self, pert, data_shape, data_loader, y_list, clean_flow_list,
                             multiplier, a_abs, eps,old_grad = None, device=None):
        ...
        with torch.no_grad():
            grad = self.normalize_grad(grad_tot)
            # self.mean_grad = 0.95*self.mean_grad + 0.05*(grad_tot**2)
            self.mean_grad = self.mean_grad_beta * self.mean_grad + (1-self.mean_grad_beta) * (grad ** 2)
            pert += (a_abs / (self.mean_grad ** 0.5)) * grad
            # pert += (a_abs/(self.mean_grad**0.5))*grad_tot
            pert = self.project(pert, eps)

        return pert

    def gradient_ascent_Adam_step_sign(self, pert, data_shape, data_loader, y_list, clean_flow_list,
                                     multiplier, a_abs, eps, old_grad=None, device=None):
        ...
        with torch.no_grad():
            ...
            self.momentum = self.beta_one * self.momentum + (1 - self.beta_one) * grad
            self.mean_grad = self.beta_two * self.mean_grad + (1 - self.beta_two) * (grad ** 2)
            pert += (a_abs / (self.mean_grad ** 0.5)+self.epsilon) * self.momentum

            pert = self.project(pert, eps)

        return pert

    def gradient_ascent_Adam_step_no_sign(self, pert, data_shape, data_loader, y_list, clean_flow_list,
                                     multiplier, a_abs, eps, old_grad=None, device=None):
        ...
        with torch.no_grad():
            self.momentum = self.beta_one*self.momentum + (1-self.beta_one)*grad_tot
            self.mean_grad = self.beta_two*self.mean_grad+(1-self.beta_two)*(grad_tot**2)
            pert += (a_abs / (self.mean_grad ** 0.5)+self.epsilon) * self.momentum
            pert = self.project(pert, eps)
        return pert

    def gradient_ascent_APGD(self, pert, data_shape, data_loader, y_list, clean_flow_list,
                                      multiplier, a_abs, eps,prev_pert = None, device=None):
        ...
        for data_idx, data in enumerate(data_loader):
            ...
            motions_gt, scale, pose_quat_gt, patch_pose, mask, perspective = extract_traj_data(data)
            mask1, mask2, perspective1, perspective2 = self.prep_data(mask, perspective)
            grad, loss = self.calc_sample_grad_APGD(pert_expand, img1_I0, img2_I0, intrinsic_I0,
                                         img1_delta, img2_delta,
                                         scale, y_list[data_idx], clean_flow_list[data_idx], patch_pose,
                                         perspective1, perspective2,
                                         mask1, mask2, device=device)
        ...
        with torch.no_grad():
            # grad = self.normalize_grad(grad_tot)
            if prev_pert is not None:
                next = pert + a_abs* grad_tot
                z = self.project(next, eps)
                pert += 0.75*(z-pert) + 0.25*(pert - prev_pert)
                pert = self.project(pert, eps)
