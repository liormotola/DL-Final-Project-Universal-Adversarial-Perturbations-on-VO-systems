...
class PGD(Attack):
    def __init__(
        ...
        super(PGD, self).__init__(model, criterion, test_criterion, norm, data_shape,mean_grad_beta,
    ...
    def perturb_adam(self, data_loader, y_list, eps,
                     targeted=False, device=None, eval_data_loader=None, eval_y_list=None):
        ...
        for rest in tqdm(range(self.n_restarts)):
            ...
            for k in tqdm(range(self.n_iter)):
                ...
                pert = self.gradient_ascent_Adam_step_sign(pert, data_shape, data_loader, y_list, clean_flow_list,
                                        multiplier, a_abs, eps, device=device)
    ...
    #
    #         pert = self.project(pert, eps)
    #         old_grad = torch.zeros_like(pert, requires_grad=False)
    #         for k in tqdm(range(self.n_iter)):
    #             print(" attack optimization epoch: " + str(k))
    #             iter_start_time = time.time()
    #
    #             pert,old_grad = self.gradient_ascent_momentum_step(pert, data_shape, data_loader, y_list, clean_flow_list,
    #                                     multiplier, a_abs, eps,old_grad, device=device)
    #
    #             step_runtime = time.time() - iter_start_time
    #             print(" optimization epoch finished, epoch runtime: " + str(step_runtime))
    #
    ...
    def perturb_APGD(self, data_loader, y_list, eps,
                                   targeted=False, device=None, eval_data_loader=None, eval_y_list=None):
        ...
        for rest in tqdm(range(self.n_restarts)):
            ...
            pert_0 = torch.zeros_like(best_pert)
            ...
            # elif self.rand_init:
            #     print(" perturbation initialized randomly")
            #     pert = self.random_initialization(pert_0, eps)
            # else:
            #     print(" perturbation initialized to zero")
            pert_1, loss_0 = self.gradient_ascent_APGD(pert_0, data_shape, data_loader, y_list, clean_flow_list,
                                        multiplier, a_abs, eps, device=device)
            pert_1 = self.project(pert_1, eps)
            f_0 = loss_0
            pert_2, loss_1 = self.gradient_ascent_APGD(pert_1, data_shape, data_loader, y_list, clean_flow_list,
                                                multiplier, a_abs, eps, device=device)
            f_1 = loss_1
            f_max = f_0
            pert_max = pert_0
            ...
            pert_k = pert_1
            w_delta= 10
            w = list(range(0,self.n_iter,w_delta))
            f_list = [f_0, f_1]
            f_max_w = f_max
            a_prev= a_abs
            ...
            pj_prev =0
            pj =0.22
            w_list2 = [0,np.ceil(pj*self.n_iter)]
            for k in range(1,self.n_iter):
                pj_1 = pj + max(pj-pj_prev-0.03, 0.06)
                pj_prev=pj
                pj=pj_1
                w_list2.append(np.ceil(pj*self.n_iter))
            # --------------------------------
            for k in tqdm(range(1,self.n_iter)):
                print(" attack optimization epoch: " + str(k))
                iter_start_time = time.time()

                pert_k_plus_1, _ = self.gradient_ascent_APGD(pert_k,data_shape, data_loader, y_list, clean_flow_list,
                                        multiplier, a_abs, eps,device=device, prev_pert = prev_pert)
                prev_pert = pert_k
                pert_k = pert_k_plus_1
                _, loss = self.gradient_ascent_APGD(pert_k, data_shape, data_loader, y_list,
                                                         clean_flow_list,
                                                         multiplier, a_abs, eps, device=device)
                f_list.append(loss)
                if loss > f_max:
                ...
                if k in w_list2:
                    a_abs, pert_k = self.halve_step_size(a_abs, a_prev, f_list, f_max_w,f_max,pert_k_plus_1, pert_max )
                    f_list = []
                    f_max_w = f_max
                step_runtime = time.time() - iter_start_time
                print(" optimization epoch finished, epoch runtime: " + str(step_runtime))
                ...
                with torch.no_grad():
                    eval_loss_tot, eval_loss_list = self.attack_eval(pert_max, data_shape, eval_data_loader, eval_y_list,
                                                                     device)

                    if eval_loss_tot > best_loss_sum:
                        best_pert = pert_max.clone().detach()
                        best_loss_list = eval_loss_list
                        best_loss_sum = eval_loss_tot
                    all_loss.append(eval_loss_list)
                    all_best_loss.append(best_loss_list)
    ...
    def halve_step_size(self, a,a_prev, f_list,f_max_prev, f_max, x_k_1 , x_max):
        ro = 0.75
        counter= sum([f_list[i+1] > f_list[i] for i in range(len(f_list)-1)])
        cond1 = counter < ro*len(f_list)
        cond2 = f_max == f_max_prev and a == a_prev
        if cond1 or cond2:
            return a/2, x_max
        else:
            return a,x_k_1
