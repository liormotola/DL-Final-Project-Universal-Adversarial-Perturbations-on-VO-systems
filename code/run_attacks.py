...
#     # train_size = int((1-validation_ratio)*len(args.testDataset))
#     #
#     # indices = np.array(range(len(args.testDataset)))
#     # np.random.shuffle(indices)
#     # train_ind, val_ind =  indices[:train_size] , indices[train_size:]
#     #
#     # dl_train = torch.utils.data.DataLoader(
#     #     dataset=args.testDataset, batch_size=args.batch_size, num_workers=args.worker_num,
...
#         attack.perturb(args.testDataloader, motions_target_list, eps=args.eps, device=args.device)#, eval_data_loader=dl_valid)
#
...
def run_attacks_train(args):
    ...
    train_indices = [traj_idx for traj_idx in traj_indices
                     if (traj_idx not in test_indices)]
    ...
    #     sampler=torch.utils.data.SubsetRandomSampler(val_ind),)
    #
    # y_train = list(itemgetter(*train_ind)(motions_target_list))
    # y_val = list(itemgetter(*val_ind)(motions_target_list))
    #
    # best_pert, clean_loss_list, all_loss_list, all_best_loss_list = \
    #     attack.perturb(dl_train, y_train, eps=args.eps, device=args.device,
    #                            eval_data_loader= dl_valid,
    #                            eval_y_list= y_val)  # , eval_data_loader=dl_valid)

    best_pert, clean_loss_list, all_loss_list, all_best_loss_list = \
        attack.perturb_adam(train_dataloader, motions_target_train_list, eps=args.eps, device=args.device, eval_data_loader= test_dataloader, eval_y_list= motions_target_test_list)#, eval_data_loader=dl_valid)
...
def run_attacks_train_oos(args):
    ...
    train_mean_delta_list = []
    train_std_delta_list = []
    eval_mean_delta_list = []
    eval_std_delta_list = []
    test_mean_delta_list = []
    test_std_delta_list = []

    train_mean_ratios_list = []
    train_std_ratios_list = []
    eval_mean_ratios_list = []
    eval_std_ratios_list = []
    test_mean_ratios_list = []
    test_std_ratios_list = []
    ...
    for test_fold_idx in folds_indices_list:
        ...
        best_pert, eval_clean_loss_list, eval_all_loss_list, eval_all_best_loss_list = \
            attack.perturb_adam(train_dataloader, motions_target_train_list, eps=args.eps, device=args.device,
        ...
        traj_adv_train_target_rms_list, traj_adv_train_target_mean_partial_rms_list = tuple(traj_adv_train_criterions_list)
        traj_adv_eval_rms_list, traj_adv_eval_mean_partial_rms_list, \
        traj_adv_eval_target_rms_list, traj_adv_eval_target_mean_partial_rms_list = tuple(traj_adv_eval_criterions_list)
        traj_adv_test_rms_list, traj_adv_test_mean_partial_rms_list, \
        traj_adv_test_target_rms_list, traj_adv_test_target_mean_partial_rms_list = tuple(traj_adv_test_criterions_list)
        ...
        train_mean_delta_list.append(frames_delta_train_rms_mean)
        train_std_delta_list.append(frames_delta_train_rms_std)
        eval_mean_delta_list.append(frames_delta_eval_rms_mean)
        eval_std_delta_list.append(frames_delta_eval_rms_std)
        test_mean_delta_list.append(frames_delta_test_rms_mean)
        test_std_delta_list.append(frames_delta_test_rms_std)


        train_mean_ratios_list.append(frames_ratio_train_rms_mean)
        train_std_ratios_list.append(frames_ratio_train_rms_std)
        eval_mean_ratios_list.append(frames_ratio_eval_rms_mean)
        eval_std_ratios_list.append(frames_ratio_eval_rms_std)
        test_mean_ratios_list.append(frames_ratio_test_rms_mean)
        test_std_ratios_list.append(frames_ratio_test_rms_std)
    ...
    with open(f"{output_dir}/train/adv_ratios.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["#", "#", "mean ratio", "#", "#"])
        writer.writerow(range(8))
        writer.writerows(train_mean_ratios_list)
        writer.writerow(["#","#","stds ratio","#","#"])
        writer.writerows(train_std_ratios_list)
        writer.writerow(["#", "#", "mean delta", "#", "#"])
        writer.writerows(train_mean_delta_list)
        writer.writerow(["#", "#", "stds delta", "#", "#"])
        writer.writerows(train_std_delta_list)

    with open(f"{output_dir}/eval/adv_ratios.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["#", "#", "mean ratio", "#", "#"])
        writer.writerow(range(8))
        writer.writerows(eval_mean_ratios_list)
        writer.writerow(["#","#","stds ratio","#","#"])
        writer.writerows(eval_std_ratios_list)
        writer.writerow(["#", "#", "mean delta", "#", "#"])
        writer.writerows(eval_mean_delta_list)
        writer.writerow(["#", "#", "stds delta", "#", "#"])
        writer.writerows(eval_std_delta_list)

    with open(f"{output_dir}/test/adv_ratios.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["#", "#", "mean ratio", "#", "#"])
        writer.writerow(range(8))
        writer.writerows(test_mean_ratios_list)
        writer.writerow(["#","#","stds ratio","#","#"])
        writer.writerows(test_std_ratios_list)
        writer.writerow(["#", "#", "mean delta", "#", "#"])
        writer.writerows(test_mean_delta_list)
        writer.writerow(["#", "#", "stds delta", "#", "#"])
        writer.writerows(test_std_delta_list)
...
def main():
    ...
    # return run_attacks_train(args)
    return run_attacks_train_oos(args)
